...
class yys_win(object):
    ...
    def goto_yuhun(self):
        ...
        if self.SETTING['win_num'] == 2:
            ...
            self.pane_yuhun4.place()
            self.config_msgbox(text='进入4人御魂...', bg='warning')
        else:
            self.pane_home.place()
...
class pane_yuhun2(child_pane):
    ...
    def do_start(self):
        self.sender.put({"type": "start", "func": "yuhun", "args":{"type":"dual"}})
...
class pane_yuhun4(child_pane):
    def init_extensions(self):
        ...
        self.btn1 = tkinter.Button(
            self.pane, text="固定窗口1", command=lambda: self.pin_win(1))
        self.btn2 = tkinter.Button(
            self.pane, text="固定窗口2", command=lambda: self.pin_win(2))
        self.btn3 = tkinter.Button(
            self.pane, text="固定窗口3", command=lambda: self.pin_win(3))
        self.btn4 = tkinter.Button(
            self.pane, text="固定窗口4", command=lambda: self.pin_win(4))
    ...
    def pin_win(self, num: int):
        self.sender.put(
            {"type": "command", "params": "pin window", "args": num})

    def do_start(self):
        self.sender.put({"type": "start", "func": "yuhun",
                         "args": {"type": "quadra"}})
